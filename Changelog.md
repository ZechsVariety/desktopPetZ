...
* Changed left/right border detection so that it can happen even while pet is ontop of a window. This is mainly so that pets still collide with eachother when on a window, but it also just makes collisions a lot more predictable
